...
public class Lexer {
	...
	/* Additional error handling tokens. */ 
	...
	public static enum TokenType {
		NAME(NAME_PATTERN), STRING(STRING_PATTERN), DATA(DATA_PATTERN), OPEN(OPEN_PATTERN), CLOSE(CLOSE_PATTERN), LTSL(
				null), SLGT(null), ASSIGN(ASSIGN_PATTERN), COMMENT(COMMENT_PATTERN), EOF(EOF_PATTERN), AMPERSAND(
						AMPERSAND_PATTERN), TEMP(TEMP_ASSIGN_PATTERN), FWCSLASHES(
								FORWARD_SLASHES_CLOSE_PATTERN), FWOSLASHES(FORWARD_SLASHES_OPEN_PATTERN);
		...
		while (m.find()) {
			if (m.group().matches(COMMENT_PATTERN) || m.group().matches(WHITESPACE_PATTERN)) {
				...
				if (m.group().contains("*") || m.group().contains("+") || m.group().contains("[")
					...
					throw new LexerException("Scanner error: Illegal character in token '" + 
								 m.group() + "'.");
				} else {
